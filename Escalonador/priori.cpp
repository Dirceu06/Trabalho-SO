...
float tempo_medio_espera(int (*lt)[3], int tam) {
    ...
    for (int p = 0; p < contagem; p++) {
        ...
        for (int t = tam - 1; t >= 0; t--) {
            if (lt[t][0] == p) {
                conclusao = t + 1; // terminou ao final dessa unidade
                break;
            }
        }
        int turnaround = conclusao - processos[p].tempo_chegada;
        int espera     = turnaround - processos[p].tempo_execucao;
        soma += espera;
    }
    return soma / contagem;
}
...
void imprimir_metricas(int (*lt)[3], int tam) {
    ...
    for (int p = 0; p < contagem; p++) {
        ...
        int resposta = conclusao - processos[p].tempo_chegada;  // turnaround
